...
## Проверка фактически загруженных библиотек
...
Рабочее устройство называется `ELAN Fingerprint Sensor (press)`. Если вместо него используется стандартный драйвер с открытым исходным кодом и устройством `ElanTech Fingerprint Sensor`, проверьте символическую ссылку TOD, правило udev и журнал.
...
Отсутствие мигания светодиода не означает, что сканер отпечатков не работает. Управление светодиодом не входит в текущий пакет.
